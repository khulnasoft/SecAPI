...
# The purpose of this file is to unify prow/lib.sh in both khulnasoft and khulnasoft.com
...
DEFAULT_KIND_IMAGE="gcr.io/khulnasoft-testing/kind-node:v1.28.4"
...
function setup_kind_cluster() {
  local NAME="${1:-khulnasoft-testing}"
...
EOF
  ...
  # Khulnasoft CI doesn't offer IPv6 connectivity, so CoreDNS should be configured
...
EOF
    ...
    # Enable core dumps
    retry docker exec "${CLUSTER_NAME}"-control-plane bash -c "sysctl -w kernel.core_pattern=/var/lib/khulnasoft/data/core.proxy && ulimit -c unlimited"
  }
