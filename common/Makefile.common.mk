...
update-common:
	...
	@rm -fr common
# khulnasoft/community has its own CONTRIBUTING.md file.
	@CONTRIB_OVERRIDE=$(shell grep -l "khulnasoft/community/blob/master/CONTRIBUTING.md" CONTRIBUTING.md)
	@if [ "$(CONTRIB_OVERRIDE)" != "CONTRIBUTING.md" ]; then\
		rm $(TMP)/common-files/files/CONTRIBUTING.md;\
	fi
# khulnasoft/khulnasoft.com uses the  Creative Commons Attribution 4.0 license. Don't update LICENSE with the common Apache license.
...
tidy-docker:
	@docker image prune --all --force --filter="label=io.khulnasoft.repo=https://github.com/khulnasoft/tools" --filter="label!=io.khulnasoft.version=$(IMAGE_VERSION)"
