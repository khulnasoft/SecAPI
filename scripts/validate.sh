...
# This is not currently a part of presubmit as there is a circular dependency on khulnasoft/khulnasoft repo
# for khulnasoftctl, which would prevent adding new fields (since they would fail the validation command).
...
for proto in $(find . -name "*.proto" | grep -v ^./common-protos | grep -v ^./envoy); do
    ...
    # then pass to khulnasoftctl validate
    < "${proto}" \
      sed 's/^\/\/ //g' | sed 's/^\/\///g' \
    ...
    | sh -c '
    cfg="$(cat < /dev/stdin)"
    echo "${cfg}" | khulnasoftctl validate -f - || { echo -e "${cfg}\n\n"  && exit 1; }
    ' || exit 1
done
